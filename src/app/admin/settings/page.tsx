...
export default function AdminSettingsPage() {
  ...
  useEffect(() => {
    fetch("/api/auth/me").then((r) => r.json()).then((d) => { setName(d.name || ""); setEmail(d.email || ""); }).catch(() => {});
  ...
  const changePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { setPwMsg({ type: "success", text: "Password changed" }); setCurrentPassword(""); setNewPassword(""); setConfirmPassword(""); }
  ...
  const saveSite = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/site-settings", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(site) });
      if (res.ok) setSiteMsg({ type: "success", text: "Site settings saved" });
      else setSiteMsg({ type: "error", text: "Failed to save" });
    } catch { setSiteMsg({ type: "error", text: "Network error" }); }
    setSiteSaving(false);
  };
  ...
  return (
    <div className="p-6 max-w-2xl space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage your account and site settings</p>
      </div>

      {/* Site Settings */}
      <form onSubmit={saveSite} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Site Information</h2>
        <p className="text-xs text-gray-400">Business info shown on the landing page and customer-facing pages.</p>
        {siteMsg && (
          <div className={`text-sm px-3 py-2 rounded-lg ${siteMsg.type === "success" ? "bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}>{siteMsg.text}</div>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Business Name</label>
            <input type="text" value={site.businessName} onChange={(e) => setSite({ ...site, businessName: e.target.value })}
          ...
          <div className="sm:col-span-2">
            ...
            <input type="text" value={site.tagline} onChange={(e) => setSite({ ...site, tagline: e.target.value })}
              className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--color-primary-light)] outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Phone</label>
            <input type="tel" value={site.phone} onChange={(e) => setSite({ ...site, phone: e.target.value })}
              className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--color-primary-light)] outline-none" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="url" value={site.facebook} onChange={(e) => setSite({ ...site, facebook: e.target.value })}
              placeholder="https://facebook.com/..."
              className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--color-primary-light)] outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Instagram URL</label>
            <input type="url" value={site.instagram} onChange={(e) => setSite({ ...site, instagram: e.target.value })}
              placeholder="https://instagram.com/..."
              className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--color-primary-light)] outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">LinkedIn URL</label>
            <input type="url" value={site.linkedin} onChange={(e) => setSite({ ...site, linkedin: e.target.value })}
      ...
      <form onSubmit={saveProfile} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
        ...
        {msg && (
          <div className={`text-sm px-3 py-2 rounded-lg ${msg.type === "success" ? "bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}>{msg.text}</div>
        )}
        <div>
      ...
      <form onSubmit={changePassword} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
        ...
        {pwMsg && (
          <div className={`text-sm px-3 py-2 rounded-lg ${pwMsg.type === "success" ? "bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}>{pwMsg.text}</div>
        )}
        <div>
